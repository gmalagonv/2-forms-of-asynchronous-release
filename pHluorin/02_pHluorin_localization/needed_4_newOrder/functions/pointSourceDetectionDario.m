...
%
% Dario Modification: change was do it to allow pass from outside de max
% loc. Line 114 was change. Extra imput:
%                  LocMax : binary max
...
function [pstruct, mask, imgLM, imgLoG] = pointSourceDetectionDario(img, sigma, varargin)
%fitGaussians2D
% Parse inputs
ip = inputParser;
...
ip.addParamValue('LocMax', [], @isnumeric);
...
if ip.Results.Prefilter
    ...
    mask = pval < 0.05;
%     mask = pval < 0.05;   %%%%% Dario
%     AvegFilter = fspecial('disk',2);                     %%%%% Dario
%     mask = imfilter(double(mask), AvegFilter, 'replicate'); %%%%% Dario
%     mask = logical(mask); %%%%% Dario
...
if isempty(ip.Results.LocMax)
    allMax = locmax2d(imgLoG, 2*ceil(sigma)+1);
else
    allMax = ip.Results.LocMax;
end
