function result = bfopen_GM3(id, startRow , startCol, lenghtRow, lenghtCol, varargin)
% bfOpen3DVolume_GM(nameMov, 0, 1, startRow , startCol, lenght, lenght);


%% GM 11.10.2020 modified to average frames to get a single image instead of isolated frames. 
...
for s = 1:numSeries
    ...
    %% GM, Nov.2019. commented out: Nov 2020
...
%% GM Nov. 2020
%     if length(indxFrames) > 1
%         imagesIndexList = indxFrames; 
%     end
%     if avgFlag == 1
%            imageList = cell(1, 1);
%            %imageList{1, 1} = zeros(size(arr), 'uint16');
%            imageList{1, 1} = zeros(Xnumpix, Ynumpix);
%     else
    imageList = cell(length(imagesIndexList), 1); %GM nov.2020
   % end


    colorMaps = cell(numImages);
    
    result = cell(length(startRow), 2);
    for syn = 1: length(startRow)
        sr = startRow(syn);
        sc = startCol(syn); 
        for i = imagesIndexList
            %if mod(i, 72) == 1 GM, Nov.2019
             %   fprintf('\n    ');
            %end        
            %fprintf('.');
...
%                
            ...
            arr = bfGetPlane(r, i, sr, sc, lenghtRow, lenghtCol);
            %end
           %% GM nov 2020 
...
%% **********************************************************************************************************
            ...
            %imageList{i, 1} = arr;
    %         if avgFlag == 1
    %             imageList{1, 1} = imageList{1, 1} + double(arr);
    %         else
            imageList{i, 1} = arr;
            
            %end
            %imageList{i, 2} = label; % GM Nov, 2020

        end
        fprintf(num2str(syn));
        result{syn, 1} = imageList;
        ...
        result{syn, 2} = seriesMetadata;
        result{syn, 3} = colorMaps;
        result{syn, 4} = r.getMetadataStore();
    end
%      if avgFlag == 1
%          imageList{1, 1} = imageList{1, 1}./length(imagesIndexList);
%      end
...
%         if avgFlag == 1
%         %fprintf('\n');
%             fprintf('AVG movie loaded baby!');
%         else
%             fprintf('ALL movie loaded baby!');
%         end
%         fprintf('\n');
%     end
end
